...
export class StudentsService {
  ...
  getAllStudents() {
    return this.http.get(`${environment.API_URL}/institucion/GetEstudiantesEnInstitucion?Institucion_ID=` + this.user.institutionId);
  }
  ...
  getSectionInfo(sectionId) {
    return this.http.get(`${environment.API_URL}/Docentes/getSeccionInfo?id=` + sectionId);
  }
  ...
  getStudentById(id: string) {
    return this.http.get(`${environment.API_URL}/Institucion/getInfoEstudiante?id=` + id + '&Institucion_ID=' + this.user.institutionId);
  }
}
